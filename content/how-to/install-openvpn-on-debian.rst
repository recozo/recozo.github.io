...
#. 初始化 easy-rsa ::
   ...
   对以下参数进行配置，以设置新证书的默认值 ::
		...
		set_var EASYRSA_REQ_CITY	"Pingxiang"
		set_var EASYRSA_REQ_ORG		"Luxi County Government"
		set_var EASYRSA_REQ_EMAIL	"careq@luxi.gov.cn"
		set_var EASYRSA_REQ_OU		"LinYeJu"
...
#. 生成CA证书 ::
   ...
   在build-ca或gen-req时会报错：
   Can't load /etc/openvpn/easy-rsa/pki/.rnd into RNG
   
   Removing the following line at the beginning of pki/openssl-easyrsa.cnf seems to resolve the issue ::

   	RANDFILE = $ENV::EASYRSA_PKI/.rnd
   
   参见 https://github.com/OpenVPN/easy-rsa/issues/261，
   以上错误是DEBIAN10的版本没有更新吗？

   以上命令会生成 ca.crt 和 ca.key 两个文件，分别代表该 CA 的公钥与私钥(私钥必须严格保密)，
   根据实际可设置 Common Name(如 LinYeJu CA)或使用默认值
...
#. 生成服务器证书 ::
	...
	$ ./easyrsa gen-req VPNSVR0520 nopass
	$ ./easyrsa sign-req server VPNSVR0520

   或者 ::

    $ ./easyrsa build-server-full VPNSVR0520 nopass

   将以上生成的服务器证书复制到 OpenVPN 目录下 ::
  
	$ sudo cp pki/private/VPNSVR0520.key /etc/openvpn/
	$ sudo cp pki/issued/VPNSVR0520.crt /etc/openvpn/
...
#. 编辑IPV4规则 ::
	...
	*filter
	...
	# 允许 freeradius 服务器访问
	# -A INPUT -i eth0 -s 10.62.9.115 -j ACCEPT
	# -A OUTPUT -o eth0 -d 10.62.9.115 -j ACCEPT
...
可以根据需要在 server.conf 中配置以下命令来指定客户端的路由设置 ::
...
使用以下内容生成 make_config.sh 内容 ::
	...
	BASE_CONFIG=/home/recozo/client-configs/base.conf
...
令牌制作方法见生成客户端证书以及配置文件 ::
	...
	$ vi ~/client-configs/tokenbase.con
...
或者 ::
	...
	$ ./easyrsa build-client-full VPNCLI0520-001 nopass
...
执行脚本生成 OVPN 文件 ::
	...
	$ ~/client-configs/make_config.sh VPNCLI0520-001

以上命令会在 ~/client-configs/files 目录下生成 VPNCLI0520-001.ovpn 文件，
